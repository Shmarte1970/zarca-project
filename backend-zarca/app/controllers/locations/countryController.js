const entityService = require('../../services/locations/countryService');
...
const get = async (req, res) => {        
    ...
    if(entity){        
        ...
        res.status(404).send({ error: 1, message: "Country not found"});
    }
};
...
const deleteEntity = async (req, res) => {        
    try {
        ...
        res.status(200).json({ success: true, message: 'Country was deleted' });
    } catch (err) {        
        res.status(400).json({ success: false, errors: err.dbErrors || err.message });
    }     
};
