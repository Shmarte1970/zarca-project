...
test.before(async t => {
	...
	t.context.modelService = require('../../services/locations/cityService');
	const {dbSetup} = require('../../config/dbConfig');
	await dbSetup();

	// Extra data for tests
	t.context.countryService = require('../../services/locations/countryService');
	await t.context.countryService.createEntity({ name: "Test"});

	t.context.provinceService = require('../../services/locations/provinceService');
	await t.context.provinceService.createEntity({ name: "Test", "countryId": 1});
});
