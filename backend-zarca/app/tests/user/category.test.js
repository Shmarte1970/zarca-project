...
test.before(async t => {
	...
	t.context.modelService = require('../../services/user/categoryService');
	const {dbSetup} = require('../../config/dbConfig');
	await dbSetup();

	// Extra data for tests
	t.context.specialityService = require('../../services/user/specialityService');
	await t.context.specialityService.createEntity({ name: "Test"});
});
